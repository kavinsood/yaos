...
function configFetch(env: Env, path: string, init?: RequestInit): Promise<Response> {
	return env.YAOS_CONFIG.get(env.YAOS_CONFIG.idFromName("global-config")).fetch(`https://internal${path}`, init);
}
...
export async function provisionReservedVault(
	...
	try {
		const runtime = env.YAOS_SYNC.get(env.YAOS_SYNC.idFromName(vault.vaultId));
		const provision = await runtime.fetch("https://internal/__yaos/provision", {
			method: "POST",
		...
		if (!provision.ok) throw new Error(`vault runtime provision failed (${provision.status})`);
