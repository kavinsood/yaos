...
## Real-device run summary (as of 2026-05-24)

| Run | Devices | Result | Artifact |
|-----|---------|--------|---------|
| s12a-local-two-vault | 2× desktop CDP | PASS 6/6 | `qa-runs/s12a-local-two-vault-pass/` |
| s12a-linux-android | Linux + real Android | PASS 6/6 | `qa-runs/s12a-linux-android-pass/` |
| s13-editor-open-remote-edit | 2× desktop CDP | PASS 6/6 | `qa-runs/s13-editor-open-remote-edit-pass/` |
| s13-linux-android | Linux + real Android | PASS 6/6 | `qa-runs/s13-linux-android-editor-open-remote-edit/` |

**Key finding from s13-linux-android**: Android open-editor remote edit is safe. Zero transient disk lag (cleaner than desktop — 4000ms mobile stability window gives DiskMirror's 1500ms open-write deferral time to complete). The "external persistence detected / merged" notification is confirmed harmless.

**transient_open_editor_disk_lag classification**: Implemented in `qa/scripts/analyze-bundles.ts` and `qa/analyzers/rules/convergence-evidence.ts`. Conditions: `disk_crdt_mismatch` + `fileOpen=true` + `editorSampleKind=healthy_sampled`. Diagnostic severity, not correctness failure. Surfaced in `diagnostic_observations` section of report.

**Pending**: s12b mobile background, s12a three-device, s12c conflict real-device.
