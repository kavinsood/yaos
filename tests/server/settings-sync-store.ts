...
function jsonBytes(value: unknown): Uint8Array { return new TextEncoder().encode(JSON.stringify(value)); }
...
s.test("seed, monotonic LWW, plugin version gates, and tombstones round-trip", () => {
	...
	assert.deepEqual(store.seed(KEY, {
		files: [{ path: "app.json", sha256: HASH, body: jsonBytes({ promptDelete: false }) }],
		intents: [intent()],
		themes: [{ name: "Minimal", repo: "kepano/obsidian-minimal", version: "7.0.0" }],
		pluginData: [{ pluginId: "calendar", pluginVersion: "1.5.10", sha256: HASH, body: jsonBytes({ startDay: 1 }) }],
	}), { ok: true, value: { envRev: 1, rev: 1 } });
	assert.deepEqual(store.putFile(KEY, "app.json", HASH, jsonBytes({ promptDelete: true })),
		{ ok: true, value: { envRev: 2, rev: 2 } });
	assert.deepEqual(store.putPluginData(KEY, { pluginId: "calendar", pluginVersion: "1.5.9", sha256: HASH, body: jsonBytes({}) }),
...
s.test("invalid seed and duplicate replace fail atomically before mutation", () => {
	...
	assert.deepEqual(failed.seed(KEY, { files: [{ path: "app.json", sha256: HASH, body: new TextEncoder().encode("{") }] }),
		{ ok: false, status: 400, error: "invalid_json" });
	assert.deepEqual(failed.getEnvironment(KEY), { ok: true, value: { seeded: false } });
	const store = makeStore();
	store.seed(KEY, { files: [{ path: "app.json", sha256: HASH, body: jsonBytes({ before: true }) }] });
	assert.deepEqual(store.replace(KEY, { files: [
		{ path: "hotkeys.json", sha256: HASH, body: jsonBytes({ one: true }) },
		{ path: "hotkeys.json", sha256: HASH, body: jsonBytes({ two: true }) },
...
s.test("duplicates and every durable string/body/count boundary fail closed", () => {
	for (const snapshot of [
		...
		{ files: [0, 1].map(() => ({ path: "app.json", sha256: HASH, body: jsonBytes({}) })) },
		{
			intents: [intent()],
			pluginData: [0, 1].map(() => ({
				pluginId: "calendar",
	...
	const store = makeStore();
	store.seed(KEY, {});
	assert.equal(store.putFile(KEY, "snippets/../app.json", HASH, jsonBytes({})).ok, false);
	...
	assert.equal(store.putFile(KEY, "app.json", "a".repeat(63), jsonBytes({})).ok, false);
	const huge = new Uint8Array(MAX_SETTINGS_FILE_BYTES + 1).fill(97);
	...
	const tooMany = makeStore().seed(KEY, { files: Array.from({ length: MAX_SETTINGS_FILES + 1 }, (_, index) => ({
		path: `snippets/${index}.css`, sha256: HASH, body: new Uint8Array([97]),
	})) });
	...
	const aggregate = makeStore().seed(KEY, { files: Array.from({ length: 5 }, (_, index) => ({
		...
		sha256: HASH,
		body: new Uint8Array(aggregateBodySize).fill(97),
	})) });
...
s.test("bounded HTTP reader rejects an oversized snapshot before seed", async () => {
	const store = makeStore();
	const response = await handleSettingsSyncRequest(store, new Request("https://internal/settings-sync/.obsidian/seed?settingsFormatVersion=2", {
		method: "PUT", headers: {
			"content-type": YAOS_BINARY_CONTENT_TYPE,
			"content-length": String(MAX_SETTINGS_SNAPSHOT_REQUEST_BYTES + 1),
		}, body: "{}",
	}), KEY, "seed");
	assert.equal(response.status, 413);
	assert.deepEqual(decodeBinaryEnvelope(new Uint8Array(await response.arrayBuffer())), { error: "request_too_large" });
	assert.deepEqual(store.getEnvironment(KEY), { ok: true, value: { seeded: false } });
});
...
s.test("HTTP mutation binds declared SHA-256 to exact body bytes", async () => {
	...
	const response = await handleSettingsSyncRequest(
		store,
		new Request("https://internal/settings-sync/.obsidian/seed?settingsFormatVersion=2", {
			method: "PUT",
	...
	assert.deepEqual(decodeBinaryEnvelope(new Uint8Array(await response.arrayBuffer())), { error: "hash_mismatch" });
	assert.deepEqual(store.getEnvironment(KEY), { ok: true, value: { seeded: false } });
});

s.test("HTTP binary envelopes round-trip raw settings bytes without base64 inflation", async () => {
	const store = makeStore();
	const body = new Uint8Array(750_000);
	for (let index = 0; index < body.length; index++) body[index] = index % 251;
	const snapshot = {
		files: [{ path: "snippets/stress.css", sha256: await sha256Hex(body), body }],
		intents: [], themes: [], pluginData: [],
	};
	const encoded = encodeBinaryEnvelope(snapshot);
	const base64JsonBytes = new TextEncoder().encode(JSON.stringify({
		...snapshot,
		files: [{ ...snapshot.files[0], legacyEncodedBody: Buffer.from(body).toString("base64"), body: undefined }],
	})).byteLength;
	assert.ok(encoded.byteLength < base64JsonBytes * 0.8, "binary transport did not remove the base64 expansion");

	const seed = await handleSettingsSyncRequest(store, new Request(
		"https://internal/settings-sync/.obsidian/seed?settingsFormatVersion=2",
		{ method: "PUT", headers: { "content-type": YAOS_BINARY_CONTENT_TYPE }, body: encoded },
	), KEY, "seed");
	assert.equal(seed.status, 200);
	assert.equal(seed.headers.get("content-type"), YAOS_BINARY_CONTENT_TYPE);

	const get = await handleSettingsSyncRequest(store, new Request(
		"https://internal/settings-sync/.obsidian?settingsFormatVersion=2",
	), KEY);
	assert.equal(get.status, 200);
	const decoded = decodeBinaryEnvelope(new Uint8Array(await get.arrayBuffer())) as {
		files: Array<{ body: Uint8Array }>;
	};
	assert.deepEqual(decoded.files[0]!.body, body);
});
await s.done();
