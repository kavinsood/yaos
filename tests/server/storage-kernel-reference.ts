...
s.check(
	...
	"schema/storage/protocol/snapshot/settings formats remain independently pinned to 10/6/8/4/2",
);
...
for (const component of ["schemaVersion", "protocolVersion"] as const) {
	const result = negotiateSyncVersions(
		CURRENT_PRODUCT_VERSIONS,
		withVersion(component, CURRENT_PRODUCT_VERSIONS[component] + 1),
	);
	s.check(
...
const storageSkew = negotiateStorageVersion(
	CURRENT_PRODUCT_VERSIONS,
	withVersion("storageFormatVersion", 1),
);
s.check(
