...
export async function claimServer(host: string): Promise<ClaimedServer> {
	...
	if (capabilities.claimed !== true || capabilities.schemaVersion !== SCHEMA_VERSION || capabilities.protocolVersion !== PROTOCOL_VERSION
...
export async function bootstrapCatalog(identity: Identity): Promise<Map<string, { bodyId: string; fileId: string; content: string }>> {
	...
	if (started.format !== "yaos-bootstrap-v2" || started.schemaVersion !== SCHEMA_VERSION) throw new Error(`wrong bootstrap format: ${JSON.stringify(started)}`);
