...
s.section("Folder-keyed schema-7 persistence");
{
	...
	s.check(
		schema6VaultIdbName("vault-1", "generation-1", folderKey)
			=== `yaos:vault-1:generation-1:${folderKey}:schema-7`,
		"schema-7 cache uses a generation-fenced namespace",
	);
	s.check(
...
s.section("Runtime uses explicit schema-7 storage and server-derived device receipts");
{
